...
Value *DfgBase::ComputeRepeat(const AnalyzedRepeat &AR, bool isVectorized, bool isPortConfig) {
  ...
  return ComputeRepeat(Parent->Query->ExpanderPtr->expandCodeFor(AR.Prime, nullptr, DefaultIP()),
...
Instruction *DfgBase::InjectRepeat(const AnalyzedRepeat &AR, Value *Val, int Port) {
  ...
  auto Expander = Parent->Query->ExpanderPtr;
...
DedicatedDfg::DedicatedDfg(DfgFile *Parent_, Loop *LI, int Unroll)
  : DfgBase(Parent_), UnrollFactor(std::max(1, Unroll)),
    Expander(*Parent->Query->ExpanderPtr) {
  ...
  if (!(Preheader = OuterMost->getLoopPreheader())) {
    MemorySSAUpdater MSSAU(Parent->Query->MSSA);
    Preheader = InsertPreheaderForLoop(OuterMost, Parent->Query->DT, Parent->Query->LI, &MSSAU, false);
  }
...
void DfgFile::InspectSPADs() {
  ...
  auto OI = OrderedInstructions(Query->DT);
  auto DT = Query->DT;

  for (auto &BB : Func) {
    if (!OI.dfsBefore(&BB.front(), Fence))
      continue;
